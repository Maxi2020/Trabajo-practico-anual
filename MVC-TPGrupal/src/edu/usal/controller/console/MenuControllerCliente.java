...
private void updateCliente() {
		...
		/*
		 * try { clienteDAO = ClienteFactory.getClienteDAO("Serializable"); Clientes
		 * clienteModificar = new Clientes (null, null, null, null, null, null,
		 * vistaCliente.PedirID("Modificar Cliente"), null, null, null, null);
		 ...
		 * if(clienteDAO.updateCliente(0, clienteModificar)) {
		 * 
		 * vistaCliente.OperacionExitosa(); } else { vistaCliente.OpcionInvalida();; } }
		 * else { vistaCliente.CancelarOperacion(); } } else {
		 * vistaCliente.LugarError("Cliente No Modificado", "No Se Encontro el Id"); } }
		 * catch (FileNotFoundException e) {
		 * vistaCliente.LugarError("Archivo No Encontrado", e.getLocalizedMessage()); }
		 * catch (IOException e) { vistaCliente.LugarError("Entrada/Salida De Datos",
